...
function UserBan({users,pagesUser , token,getUser ,sort}) {
  ...
  async function handleStatusProvider (id) {
    let result = await apiClientPatchUser(`/admin/users/${id}/not_ban` , token);
    ...
    return (
        <MainAdminContent>
        <MainAdminAllUser>
          <h3>Tất Cả Người Dùng Bị Ban</h3>
          <MainAdminFlex>
            <MainAdmintextfunction>
        ...
        <MainAdminTable>
          ...
          <tbody>
            {(users.length !== 0)  ? users.map((user)=>(
               ...
               <td>
                 <ButtonBan  onClick={() => handleStatusProvider( user.id)}>Được Phép</ButtonBan>
               </td>
             </tr>
            )) : (
              <tr><td>
