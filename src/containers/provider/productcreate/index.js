  ...
  const onSubmit = async (data) => {
    ...
    try {
      let result = await axios.post(
        `${process.env.REACT_APP_API}/provider/create_product`,
        {
          name: data.name,
      ...
      <div style={{ margin: "30px" }}>
        <form onSubmit={handleSubmit(onSubmit)}>
          <InputControls>
            ...
            />
            {errors.name && (
              <div style={{ marginLeft: "180px" }}>{errors.name.message}</div>
            )}
          </InputControls>
          <InputControls>
          ...
          <ContainerInputFile>
            <input
              ...
              value={imgUrl}
              onChange={(e) => setImgUrl(imgUrl)}
            />
            <InputFile
              inputTitle="chọn hình ảnh sản phẩm 1"
          ...
          <ContainerInputFile>
            <input
              style={{ visibility: "hidden" }}
              value={imgUrl1}
              onChange={(e) => setImgUrl1(imgUrl1)}
            />
            <InputFile
              inputColor="chọn hình ảnh sản phẩm 2"
              style={{ marginLeft: "15px" }}
              type="file"
              onChange={handleChange}
              placeholder="dlfgjdfl"
          ...
          <ContainerInputFile>
            <input
              style={{ visibility: "hidden" }}
              value={imgUrl2}
              onChange={(e) => setImgUrl2(imgUrl2)}
            />
            <InputFile
              inputTitle="chọn hình ảnh sản phẩm 3"
              style={{ marginLeft: "15px" }}
              type="file"
              onChange={handleChange}
              placeholder="dlfgjdfl"
          ...
          <ContainerInputFile>
            <input
              style={{ visibility: "hidden" }}
              value={imgUrl3}
              onChange={(e) => setImgUrl3(imgUrl3)}
            />
            <InputFile
              inputTitle="chọn hình ảnh sản phẩm 4"
              style={{ marginLeft: "15px" }}
              type="file"
              onChange={handleChange}
              placeholder="dlfgjdfl"
